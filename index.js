 ...
 */
const composeSetupUriParamsObj = {
  categoryId: "",
  password: "12312123",
  reserved: 0,
  setupId: "AAA",
...
const composeQrCode = ({ pairingCode, setupUri }) => {
  ...
  qrImg.addEventListener("load", () => {
    ...
    const svgNode = `<?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
<svg style="width:80%;" viewBox="0 0 400 540" xmlns="http://www.w3.org/2000/svg">
  ...
  <defs>
    ...
    <symbol id="qrCode" ${setupUri.slice(5, -7)}</symbol>
    <image href="${imgSrc}" x="30" y="175" width="340" height="340"/>
  </defs>
...
async function scanAndConnect() {
  ...
  for (const char of chars) {
    try {
      // 读取 0x2901 描述符（User Description）
      const desc = await char.getDescriptor(
        "00002901-0000-1000-8000-00805f9b34fb"
      );
      const value = await desc.readValue();
      const endpoint = new TextDecoder().decode(value.buffer).trim();
      console.log(endpoint);
      if (
        ["prov-session", "prov-config", "proto-ver", "targetMAC"].includes(
          endpoint
        )
      ) {
        characteristics[endpoint] = char;
...
async function sendProvision() {
  const ssid = dom_ssidInput.value;
  const pwd = dom_passwordInput.value;
  const targetMAC = dom_targetMAC.value;
  // 加载 proto
  const root = await protobuf.load("wifi_config.proto");
  const WiFiConfigPayload = root.lookupType("WiFiConfigPayload");
  const CmdSetConfig = root.lookupType("CmdSetConfig");
  const WiFiConfigMsgType = root.lookupEnum("WiFiConfigMsgType");
  ...
  // 立即写入 targetMAC（确保 BLE 断开前写入, 以 JSON 格式）
  if (characteristics["targetMAC"]) {
    const json = JSON.stringify({ target_mac: targetMAC });
    const encoder = new TextEncoder();
    const macBuf = encoder.encode(json);
    await ensureConnected();
    await characteristics["targetMAC"].writeValue(macBuf);
    log("已向 targetMAC 发送: " + targetMAC);
  }

  await ensureConnected();
  const respValue = await characteristics["targetMAC"].readValue();
  const respBuf = new Uint8Array(respValue.buffer);
  const respStr = new TextDecoder().decode(respBuf);
  Object.assign(composeSetupUriParamsObj, JSON.parse(respStr));
  log("收到 targetMAC 响应: ");
  log(JSON.stringify(composeSetupUriParamsObj, null, 2));

  // 构造 CmdSetConfig
  const cmdSetConfig = CmdSetConfig.create({
    ssid: new TextEncoder().encode(ssid),
    passphrase: new TextEncoder().encode(pwd),
    bssid: new Uint8Array([]), // 仍然传空，标准配网
    channel: 0,
  });

  // 构造 WiFiConfigPayload，字段名用驼峰，msg 用枚举
  const payloadSet = WiFiConfigPayload.create({
    msg: WiFiConfigMsgType.values.TypeCmdSetConfig,
    cmdSetConfig: cmdSetConfig,
  });
  const bufSet = WiFiConfigPayload.encode(payloadSet).finish();
  await ensureConnected();
  await characteristics["prov-config"].writeValue(bufSet);
  log("配网数据已写入，等待设备响应...");

  // 读取响应并判断
  try {
    await ensureConnected();
    const resp = await characteristics["prov-config"].readValue();
    const respBuf = new Uint8Array(resp.buffer);
    const respMsg = WiFiConfigPayload.decode(respBuf);
    log("收到设备响应: 设备正在重启中");
    if (
      respMsg?.respSetConfig?.status === 0 ||
      respMsg?.respGetStatus?.staState === 1 // 1=Connecting
    ) {
      const payloadApply = WiFiConfigPayload.create({
        msg: WiFiConfigMsgType.values.TypeCmdApplyConfig,
        cmdApplyConfig: {},
      });
      const bufApply = WiFiConfigPayload.encode(payloadApply).finish();
      await ensureConnected();
      await characteristics["prov-config"].writeValue(bufApply);
      log("已发送 CmdApplyConfig，等待设备连接 WiFi...");
    }
  } catch (e) {
    console.log("读取响应失败 设备重启成功");
  }
}
...
function setErrorFor(input, message) {
  const formControl = input.parentElement;
  const formTip = formControl.querySelector(".form-tip");
  formControl.className = "form-control error";
  formTip.innerText = message;
}

function validate() {
  let isValid = false;
  if (dom_ssidInput.value === "") {
    setErrorFor(dom_ssidInput, "SSID不能为空");
  } else {
    setSuccessFor(dom_ssidInput);
    isValid = true;
  }
  isValid = false;
  if (dom_passwordInput.value === "") {
    setErrorFor(dom_passwordInput, "密码不能为空");
  } else if (dom_passwordInput.value.length < 8) {
    setErrorFor(dom_passwordInput, "密码长度不能小于8");
  } else {
    setSuccessFor(dom_passwordInput);
    isValid = true;
  }

  isValid = false;
  if (targetMAC.value === "") {
    setErrorFor(dom_targetMAC, "目标 MAC 不能为空");
  }

  // AA:BB:CC:DD:EE:FF
  const macRegex = /^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/;
  if (!macRegex.test(targetMAC.value)) {
    setErrorFor(
      dom_targetMAC,
      "目标 MAC 格式不正确 （AA:BB:CC:DD:EE:FF）或 (AA-BB-CC-DD-EE-FF)"
    );
  } else {
    setSuccessFor(dom_targetMAC);
    isValid = true;
  }
  return isValid;
}

dom_mainForm.addEventListener("submit", (e) => e.preventDefault());

dom_connect_btn.addEventListener("click", async (e) => {
  await scanAndConnect();
});

dom_mainForm.addEventListener("submit", async (e) => {
  e.preventDefault();
  ...
  await sendProvision();
  composeQrCode({
    pairingCode: composeSetupUriParamsObj.password,
    setupUri: composeSetupUri(composeSetupUriParamsObj),
  });
});
